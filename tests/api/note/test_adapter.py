...
async def test_create_note(inmemory_db_session: AsyncSession, note_model: NoteModel, note: Note):
    # Use the provided in-memory session; insert a note and verify adapter
    adapter = NoteDbAdapter(db=inmemory_db_session)

    returned_result = await adapter.create_note(note=note_model)
...
async def test_update_note(
    ...
    await adapter.create_note(note=note_model)

    returned_result = await adapter.update_note(update_note)
...
async def test_delete_note(inmemory_db_session: AsyncSession, note_model: NoteModel):
    adapter = NoteDbAdapter(db=inmemory_db_session)

    created = await adapter.create_note(note=note_model)

    statement = select(Note).where(Note.id == created.id)
    execute_result = await inmemory_db_session.execute(statement)
    result = execute_result.scalar_one_or_none()
    assert result is not None

    # delete the created note and verify it's removed from the session
    await adapter.delete_note(note_id=created.id, user_id=DUMMY_USER_ID)
